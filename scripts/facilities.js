    ...
    for (const bridgeMineral of mineralsAtFacility) {
        for (const mineral of allMinerals) {
            if(mineral.id === bridgeMineral.mineralId) {
                ...
                if (amount > 0) {
                    html += `<input type="radio" class="radio--button" id="button-${mineral.id}" name="mineralId" value="${mineral.id}"/><label for="button-${mineral.id}">${amount} tons of ${mineral.name}</label><br>`
                }
            }
        }
    }
